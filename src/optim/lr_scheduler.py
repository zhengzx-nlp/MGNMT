...
class LearningRateScheduler(object):
    ...
    def __init__(self, optimizer, min_lr=-1.0, start_step=0):
        """
        Args:
    ...
    def step(self, **kwargs):
        if kwargs["global_step"] < self.start_step:
            return
...
class NoamScheduler(LearningRateScheduler):
    ...
    def __init__(self, optimizer, d_model, warmup_steps, min_lr=-1.0, start_step=0):
        super(NoamScheduler, self).__init__(
            optimizer=optimizer, min_lr=min_lr, start_step=start_step)
...
class ReduceOnPlateauScheduler(LearningRateScheduler):
    ...
    def __init__(self, optimizer, patience, min_lr=-1.0, scale=0.5, mode="min", start_step=0):

        super(ReduceOnPlateauScheduler, self).__init__(
            optimizer=optimizer, min_lr=min_lr, start_step=start_step)
    ...
    def update_lr(self, old_lr, metric, **kwargs):
